...
public class SlideBottomPanel extends FrameLayout {
    ...
    private static int DEFAULT_TITLE_HEIGHT_NO_DISPLAY = 60;
    private static int DEFAULT_PANEL_HEIGHT = 380;
    ...
    public void setDefault(int display,int height){
    	DEFAULT_TITLE_HEIGHT_NO_DISPLAY =display;
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        for (int i = 0; i < mChildCount; i++) {
            ...
            if (childView.getTag() == null || Integer.parseInt(childView.getTag().toString()) != TAG_BACKGROUND) {
...
//                }
            } else if (Integer.parseInt(childView.getTag().toString()) == TAG_BACKGROUND){
...
//            NewButton btn=(NewButton)mDarkFrameLayout.findViewById(R.id.id_text);
//            btn.setText("test");
        }
    }
    ...
    private boolean handleActionDown(MotionEvent event) {
        ...
        if (!isPanelShowing && downY > mMeasureHeight - mTitleHeightNoDisplay) {//panel还未示，点击panel某一处开始移动panel，此时listview的点击事件无效，dispatch返回true
            isPanelOnTouch = true;
            isConsume = true;
        } else if (!isPanelShowing && downY <= mMeasureHeight - mTitleHeightNoDisplay) {//panel还未示，点击空白处不做操作
            isPanelOnTouch = false;
        } else if (isPanelShowing && downY > mMeasureHeight - mPanelHeight) {//panel已经显示，点击panel某一处开始移动panel
            isPanelOnTouch = true;
        } else if (isPanelShowing && downY < mMeasureHeight - mPanelHeight) {//panel已经显示，点击空白处隐panel
            hidePanel();
            isPanelOnTouch = false;
        }
        return isConsume;
