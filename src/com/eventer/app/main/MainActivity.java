	...
	public void setAlarmList() {
		...
		while (c.moveToNext()) {
			...
			case 2:
				...
				break;
			case 3:
				if (today_dt.getWeekDay() == Remind_dt.getWeekDay()
				&& today_time[0].compareTo(remind_time[0]) >= 0) {
					IsTodayEvent = true;
				}
				break;
			case 4:
				if (today_dt.getDay() == Remind_dt.getDay()
				&& today_time[0].compareTo(remind_time[0]) >= 0) {
			...
			try {
				...
				if (mid.equals("ADD")) {
					...
					if (type == GROUP_CREATED_NOTIFICATION) {
						...
						notifyMsg("收到来自群组的消息！", "消息通知", id + "发送消息:"
								+ bodyString, intent1, 49);
					...
					dao.updateUsers(delFriend);
						...
						&& !chat_talker.equals(id)
						&& MyApplication.getInstance().getContactIDList()
						.contains(id)) {
					// 单聊消息
					recvJs = JSONObject.parseObject(msg);
					String bodyString = recvJs.getString("data");
					...
					intent1.putExtra("userId", id);
					notifyMsg("收到来自好友的消息！", "消息通知", id + "发送消息:" + bodyString,
							intent1, 33);
					ChatEntity entity = new ChatEntity();
					entity.setType(type);
					entity.setFrom(id);
